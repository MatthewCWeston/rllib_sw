...
missile_space = RepeatedCustom(Box(-1,1,shape=(Missile.REPR_SIZE,)), NUM_MISSILES)
...
class SW_1v1_env_singleplayer(gym.Env):
    def __init__(self, env_config={}):
        ...
        self.missile_space = missile_space
        obs_space = {
    ...
    def shaped_reward(self, ship, target, base_reward):
        ...
        # Reward for staying alive (small positive per step)
        r += 0.0002
        
        # Penalty proportional to proximity to star (teaches orbital stability)
        dist_to_star = np.linalg.norm(ship.pos)
        if dist_to_star < STAR_SIZE * 3:
            r -= 0.005 * (1 - dist_to_star / (STAR_SIZE * 3))
        
        # Small reward for missiles getting closer to target (aim training)
        for m in self.missiles:
            dist = np.linalg.norm(m.pos - target.pos)
            r += 0.0005 / (dist + 0.1)
        
        # Penalty for getting hit by hostile missiles (proximity warning)
